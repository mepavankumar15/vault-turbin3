...
pub struct Make<'info> {
    ...
    #[account(
        mut,
        constraint = maker_token.mint == maker_mint.key()
    )]
...
impl<'info> Make<'info> {
    ...
    pub fn save_escrow(&mut self, seed: u64, receive: u64, bumps: &MakeBumps) -> Result<()> {
        ...
        // We set the deposit amount based on the vault's balance after transfer
        self.escrow.deposit = self.vault.amount; 
...
pub struct Refund<'info> {
    ...
    #[account(
        mut,
        constraint = maker_token.mint == escrow.maker_mint
    )]
    pub maker_token: Account<'info, TokenAccount>,
    #[account(
        mut,
        close = maker, // Close escrow, refund SOL rent to maker
...
pub struct Take<'info> {
    ...
    #[account(
        mut,
        constraint = taker_token.mint == escrow.taker_mint
    )]
    pub taker_token: Account<'info, TokenAccount>,
    #[account(
        mut,
        constraint = taker_receive_token.mint == escrow.maker_mint
    )]
    pub taker_receive_token: Account<'info, TokenAccount>,
    /// CHECK: We just need this account to transfer tokens to.
    #[account(mut)]
...
impl<'info> Take<'info> {
    // Taker deposits their payment *to the maker*
    ...
    // Withdraw from vault to taker and close
    ...
    // 1. Transfer from vault to taker
    let cpi_transfer = Transfer {
        from: self.vault.to_account_info(),
